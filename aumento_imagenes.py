...
class Data_Augmentation:
    ...
    def load_data(self):
        ...
        selected_files = random.sample(image_files, 370)
    ...
    def run(self, n_proceso):
        ...
        operations = [
             ...
             self.saturacion,
             #self.girar_90
             self.girar_180,  # Agrega la función de rotación
    ...
    def ruido(self, data):
        new_data = {}
        src = data["image"]
        noise_img = random_noise(src, mode="gaussian",var=0.08)  # Generar ruido gaussiano con varianza 0.01 (ajustar según sea necesario)
        noisy_image = np.array(255 * noise_img, dtype="uint8")
        noisy_image = cv2.addWeighted(src, 0.8, noisy_image, 0.2, 0)  # Combinar imagen original y ruido
    ...
    def girar_90(self, data):
        new_data = {}
        image = data["image"]
        bbs = data["bounding_boxes"]

        # Girar la imagen 90 grados en sentido horario
        ...
        for bb in bbs:
            ...
            # Calcular las nuevas coordenadas después de la rotación
            x_center_new = 1.0 - y_center
            y_center_new = x_center
    ...
    def ruido(self, data):
        new_data = {}
        src = data["image"]
        noise_img = random_noise(src, mode="gaussian",
                                 var=0.04)  # Generar ruido gaussiano con varianza 0.01 (ajustar según sea necesario)
        noisy_image = np.array(255 * noise_img, dtype="uint8")
        noisy_image = cv2.addWeighted(src, 0.5, noisy_image, 0.7, 0)  # Combinar imagen original y el ruido
        new_data["image"] = noisy_image
        new_data["bounding_boxes"] = data["bounding_boxes"]
        return new_data
...
carpeta_imagenes = r'D:\data\JunioAumento\dataset3\dataset4\460_propio_gira90p_510\images'
carpeta_etiquetas = r'D:\data\JunioAumento\dataset3\dataset4\460_propio_gira90p_510\labels'
nueva_carpeta_imagenes = r'D:\data\JunioAumento\dataset3\dataset4\aumento\images'
nueva_carpeta_etiquetas = r'D:\data\JunioAumento\dataset3\dataset4\aumento\labels'
